...
class CheapestRoutingOptimizer {
 private:
  ...
  bool findCheapestPath(std::vector<int>& path, int& bottleneck,
                        double& pathCost) {
 ...
 public:
  ...
  bool addEdge(const std::string& fromCity, const std::string& toCity,
               int capacity) {
  ...
  bool setSourceTargetData(const std::string& src, const std::string& tgt,
                           int data) {
  ...
  // Get all city names
  std::vector<std::string> getAllCities() const { return idToCity; }
...
class UI {
 private:
  ...
  // Focus management
  int activeInputField;  // 0=city, 1=source, 2=target, 3=data, 4=edgeFrom,
                         // 5=edgeTo, 6=edgeCap, -1=none
  ...
  void calculateNodePositions(int numNodes) {
    ...
    for (int i = 0; i < numNodes; i++) {
      ...
      nodePositions.push_back(
          {centerX + radius * cosf(angle), centerY + radius * sinf(angle)});
    }
  }
  ...
  void drawGraph(bool highlightPath = false) {
    ...
    // Get the path to highlight (from iteration history if available, otherwise
    // from current path)
    ...
    if (highlightPath) {
      ...
      if (!history.empty() && currentIter > 0 &&
          currentIter <= (int)history.size()) {
    ...
    for (int i = 0; i < optimizer.getNumNodes(); i++) {
      for (const Edge& e : graph[i]) {
        if (e.initial_capacity > 0) {  // Only draw forward edges
          ...
          // Draw capacity label
          std::string capStr = std::to_string(e.residual_capacity) + "/" +
                               std::to_string(e.initial_capacity);
          Vector2 midPoint = {(fromPos.x + toPos.x) / 2.0f,
                              (fromPos.y + toPos.y) / 2.0f};
          Vector2 textSize =
              MeasureTextEx(GetFontDefault(), capStr.c_str(), 10, 1.0f);
          DrawRectangle(midPoint.x - textSize.x / 2 - 2,
                        midPoint.y - textSize.y / 2 - 2, textSize.x + 4,
                        textSize.y + 4, WHITE);
          DrawText(capStr.c_str(), midPoint.x - textSize.x / 2,
                   midPoint.y - textSize.y / 2, 10, BLACK);
        }
      }
    }
    ...
    for (int i = 0; i < optimizer.getNumNodes(); i++) {
      ...
      if (highlightPath && !pathToHighlight.empty()) {
        isPathNode = std::find(pathToHighlight.begin(), pathToHighlight.end(),
                               i) != pathToHighlight.end();
      }
      ...
      // Draw city name
      Vector2 textSize =
          MeasureTextEx(GetFontDefault(), cityName.c_str(), 14, 1.0f);
      DrawText(cityName.c_str(), pos.x - textSize.x / 2, pos.y - textSize.y / 2,
               14, BLACK);
    }
  }
  ...
  void drawStatusPanel() {
    ...
    std::string status =
        "Data Requested: " + std::to_string(optimizer.getTotalData()) +
        " | Data Transferred: " +
        std::to_string(optimizer.getDataTransferred()) +
  ...
  void drawSetupConfig() {
    ...
    DrawText("Setup Configuration - Step 1: Map Cities", 250, 100, 24,
             DARKGRAY);
    ...
    // Load 50-city demo button
    Rectangle demo50Button = {500, 280, 200, 50};
    Color demo50ButtonColor =
        CheckCollisionPointRec(GetMousePosition(), demo50Button) ? GRAY : GREEN;
    DrawRectangleRec(demo50Button, demo50ButtonColor);
    DrawRectangleLinesEx(demo50Button, 2, BLACK);
    DrawText("Load 50-City Demo", 505, 300, 18, WHITE);

    // Status message
    DrawText(statusMessage.c_str(), 50, 340, 18, statusColor);
    ...
    if (optimizer.getNumNodes() > 0) {
      ...
      Color continueButtonColor =
          CheckCollisionPointRec(GetMousePosition(), continueButton) ? GRAY
                                                                     : BLUE;
  ...
  void drawSelectSourceTarget() {
    ...
    DrawText((sourceInputText.empty() ? "Select..." : sourceInputText.c_str()),
             210, 152, 18, BLACK);
    DrawText("▼", 380, 152, 16, DARKGRAY);

    if (sourceDropdownOpen && optimizer.getNumNodes() > 0) {
      const auto& cities = optimizer.getAllCities();
      float dropdownHeight = cities.size() * 25.0f + 10.0f;
      Rectangle dropdownRect = {200.0f, 175.0f, 200.0f, dropdownHeight};
      DrawRectangleRec(dropdownRect, WHITE);
      DrawRectangleLinesEx(dropdownRect, 2, DARKGRAY);

      for (int i = 0; i < (int)cities.size(); i++) {
        float itemY = 177.0f + i * 25.0f;
        Rectangle itemRect = {202.0f, itemY, 196.0f, 23.0f};
        Color itemColor = CheckCollisionPointRec(GetMousePosition(), itemRect)
                              ? LIGHTGRAY
                              : WHITE;
        DrawRectangleRec(itemRect, itemColor);
        DrawText(cities[i].c_str(), 210, static_cast<int>(itemY + 3), 16,
                 BLACK);
      }
    }
    ...
    DrawText((targetInputText.empty() ? "Select..." : targetInputText.c_str()),
             210, 202, 18, BLACK);
    ...
    if (targetDropdownOpen && optimizer.getNumNodes() > 0) {
      const auto& cities = optimizer.getAllCities();
      float dropdownHeight = cities.size() * 25.0f + 10.0f;
      Rectangle dropdownRect = {200.0f, 225.0f, 200.0f, dropdownHeight};
      DrawRectangleRec(dropdownRect, WHITE);
      DrawRectangleLinesEx(dropdownRect, 2, DARKGRAY);

      for (int i = 0; i < (int)cities.size(); i++) {
        float itemY = 227.0f + i * 25.0f;
        Rectangle itemRect = {202.0f, itemY, 196.0f, 23.0f};
        Color itemColor = CheckCollisionPointRec(GetMousePosition(), itemRect)
                              ? LIGHTGRAY
                              : WHITE;
    ...
    DrawText(
        (dataInputText.empty() ? "Enter amount..." : dataInputText.c_str()),
        260, 252, 18, BLACK);
    ...
    if (!sourceInputText.empty() && !targetInputText.empty() &&
        !dataInputText.empty()) {
  ...
  void drawInputEdges() {
    ...
    Color addButtonColor =
        CheckCollisionPointRec(GetMousePosition(), addButton) ? GRAY : DARKGRAY;
    ...
    Color nextButtonColor =
        CheckCollisionPointRec(GetMousePosition(), nextButton) ? GRAY : GREEN;
  ...
  void drawSimulationStep() {
    ...
    bool canContinue =
        (optimizer.getDataRemaining() > 0 && optimizer.checkPathExists());
    ...
    if (canContinue) {
      ...
      // Final iteration completed - show completion message and View Results
      // button IMPORTANT: The final iteration is displayed above and remains
      // visible User must explicitly click "View Results" to proceed
      ...
      Color viewResultsButtonColor =
          CheckCollisionPointRec(GetMousePosition(), viewResultsButton) ? GRAY
                                                                        : BLUE;
  ...
  void drawResultFinal() {
    ...
    y += 30;
    DrawText(
        ("Total Data Requested: " + std::to_string(optimizer.getTotalData()))
            .c_str(),
        50, y, 18, BLACK);
    y += 25;
    DrawText(("Total Data Transferred: " +
              std::to_string(optimizer.getDataTransferred()))
                 .c_str(),
             50, y, 18, BLACK);
    y += 25;
    DrawText(("Data Remaining: " + std::to_string(optimizer.getDataRemaining()))
                 .c_str(),
             50, y, 18, BLACK);
    y += 25;
    DrawText(
        ("Total Iterations: " + std::to_string(optimizer.getCurrentIteration()))
            .c_str(),
        50, y, 18, BLACK);
    y += 25;
    ...
    float efficiency = optimizer.getTotalData() > 0
                           ? (100.0f * optimizer.getDataTransferred() /
                              optimizer.getTotalData())
                           : 0.0f;
    DrawText(("Transfer Efficiency: " + std::to_string((int)efficiency) + "%")
                 .c_str(),
             50, y, 18, BLACK);
    y += 40;
    ...
    if (!history.empty()) {
      ...
      for (size_t i = 0; i < history.size(); i++) {
        ...
        // Iteration number
        DrawText(("Iteration " + std::to_string(iter.iteration) + ":").c_str(),
                 50, currentY, 16, DARKBLUE);
  ...
  void processSetupConfig() {
    ...
    // Map Cities button
    Rectangle mapButton = {50, 230, 150, 40};
    if (CheckCollisionPointRec(GetMousePosition(), mapButton) &&
        IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      if (optimizer.initializeCities(cityInputText)) {
        statusMessage =
            "Cities mapped successfully! Click 'Input Edges' to continue.";
        statusColor = GREEN;
    ...
    // Load 5-city demo button
    Rectangle demoButton = {500, 220, 200, 50};
    if (CheckCollisionPointRec(GetMousePosition(), demoButton) &&
        IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      loadDemo();
    }
    ...
    // Load 50-city demo button
    Rectangle demo50Button = {500, 280, 200, 50};
    if (CheckCollisionPointRec(GetMousePosition(), demo50Button) &&
        IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      loadDemo50();
    }
    ...
    if (optimizer.getNumNodes() > 0) {
      Rectangle continueButton = {600, 280, 150, 40};
      if (CheckCollisionPointRec(GetMousePosition(), continueButton) &&
          IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
  ...
  void processSelectSourceTarget() {
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      ...
      // Handle source dropdown items
        ...
        const auto& cities = optimizer.getAllCities();
        for (int i = 0; i < (int)cities.size(); i++) {
          Rectangle itemRect = {202.0f, 177.0f + i * 25.0f, 196.0f, 23.0f};
      ...
      // Handle target dropdown items
        ...
        const auto& cities = optimizer.getAllCities();
        for (int i = 0; i < (int)cities.size(); i++) {
          Rectangle itemRect = {202.0f, 227.0f + i * 25.0f, 196.0f, 23.0f};
          if (CheckCollisionPointRec(mousePos, itemRect)) {
      ...
      // Handle source dropdown scrolling
      ...
      if (!sourceInputText.empty() && !targetInputText.empty() &&
        ...
        if (CheckCollisionPointRec(mousePos, continueButton)) {
          try {
            ...
            if (optimizer.setSourceTargetData(sourceInputText, targetInputText,
                                              data)) {
  ...
  void processInputEdges() {
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      ...
      if (CheckCollisionPointRec(mousePos, fromBox))
        activeInputField = 4;
      else if (CheckCollisionPointRec(mousePos, toBox))
        activeInputField = 5;
      else if (CheckCollisionPointRec(mousePos, capBox))
        activeInputField = 6;
    }
    ...
    if (key > 0 && key < 128) {
      ...
      if (activeInputField == 4)
        activeField = &edgeFromText;
      else if (activeInputField == 5)
        activeField = &edgeToText;
      else if (activeInputField == 6)
        activeField = &edgeCapacityText;
    ...
    if (IsKeyPressed(KEY_BACKSPACE)) {
      if (activeInputField == 4 && !edgeFromText.empty())
        edgeFromText.pop_back();
      else if (activeInputField == 5 && !edgeToText.empty())
        edgeToText.pop_back();
      else if (activeInputField == 6 && !edgeCapacityText.empty())
        edgeCapacityText.pop_back();
    }
    ...
    if (CheckCollisionPointRec(GetMousePosition(), addButton) &&
        IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      if (!edgeFromText.empty() && !edgeToText.empty() &&
          !edgeCapacityText.empty()) {
        try {
          ...
          if (optimizer.addEdge(edgeFromText, edgeToText, capacity)) {
            ...
            statusMessage =
                "Error: Invalid edge. Check city names and capacity.";
            statusColor = RED;
          }
        } catch (...) {
  ...
  void processSimulationStep() {
    ...
    if (CheckCollisionPointRec(GetMousePosition(), nextButton) &&
      ...
      if (optimizer.runStep()) {
        ...
        // IMPORTANT: Even if this is the final iteration, stay in
        // SIMULATION_STEP to display it. The next frame will check
        // buttonVisible and show completion message, but iteration will still
        // be visible.
      } else {
        // Step failed (no path found or no data remaining)
        // Stay in SIMULATION_STEP to show the final iteration that was just
        // completed The drawSimulationStep() will show completion message User
        // can see the final iteration before any transition
      }
    }
    ...
    if (!buttonVisible && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      // Check if user clicked on a "View Results" area (if we add such a
      // button) For now, we'll add a button in drawSimulationStep() when
      // !buttonVisible
      Rectangle viewResultsButton = {600, 500, 150, 40};
  ...
  void loadDemo() {
    ...
    statusMessage =
        "Demo graph loaded with default edges! Please select Source, Target, "
        "and Data amount.";
    statusColor = GREEN;
  ...
  void loadDemo50() {
    ...
    optimizer.initializeCities(cityList);
    calculateNodePositions(50);

    std::set<std::pair<int, int>> edgeSet;
    auto addUniqueEdge = [&](int u, int v, int cap) {
      if (u == v) return;
      if (edgeSet.count({u, v}) > 0) return;
      optimizer.addEdge(cities[u], cities[v], cap);
      edgeSet.insert({u, v});
    };

    // Guarantee a fixed path from 1 to 50.
    for (int i = 0; i < 49; i++) {
      addUniqueEdge(i, i + 1, 150 + (i % 51));
    }

    const int offsets[5] = {3, 7, 13, 21, 34};
    for (int i = 0; i < 50 && (int)edgeSet.size() < 120; i++) {
      for (int j = 0; j < 5 && (int)edgeSet.size() < 120; j++) {
        int v = (i + offsets[j]) % 50;
        addUniqueEdge(i, v, 100 + ((i * (j + 1) * 11) % 101));
      }
    }

    cityInputText = cityList;
    statusMessage =
        "50-city demo loaded with fixed edges! Select Source, Target, and Data "
        "amount.";
    statusColor = GREEN;
    sourceInputText = "";
    targetInputText = "";
